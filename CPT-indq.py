...
collision_count_cptIndQ = np.zeros(simulation_number)
cost_count_cptIndQ = np.zeros(simulation_number)

print('Simulation start')

for z in range(0,simulation_number):
    collision_count_cptIndQ[z], cost_count_cptIndQ[z] = multi_simulate_runs(pi_proposed, list(range(n_agents)), TP, R )


# %%
# save data
# save the number of collisions of 100 trajectories of CPT-Individual Q Values
fname = 'collision_count_cptIndQ.txt'
np.savetxt(fname, collision_count_cptIndQ)

# save the cost of 100 trajectories of CPT-Individual Q Values
fname = 'cost_count_cptIndQ.txt'
np.savetxt(fname, cost_count_cptIndQ)
